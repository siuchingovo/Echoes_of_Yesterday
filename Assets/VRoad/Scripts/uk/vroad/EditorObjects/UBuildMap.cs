...
namespace uk.vroad.EditorObjects
{
    ...
    public class BuildFlags : Inspectable
    {
        ...
        [Tooltip("If true, the road network can be partitioned into discrete areas, such that all destinations " +
                 "are not reachable from all origins")]
        [ProOnly]
        public bool roadNetworkPartitions = BF.Partitions.IsTrue();

        [Tooltip("If true, this keeps the native (local) name of an area, rather than translating to a locale-specific name")]
        [ProOnly]
        public bool nativeNames = BF.NativeNames.IsTrue();
    ...
    [System.Serializable] public class BuildParameters : Inspectable
    {
        ...
        [Range(0, 100)] [Tooltip("The % share of cars in the simulation, the remainder will be trucks and coaches")]
        public int carsToOthers = BP.CarsToOthers.IntValue();

        [Range(0, 100)] [Tooltip("For non-car vehicles, the % share of coaches, the remainder will be trucks")]
        public int coachesToTrucks = BP.CoachesToTrucks.IntValue();

        [Range(0, 9999)] [Tooltip("Maps of an area (in Hectares) of less than this value will be named by street and suburb. " +
                                  "Larger maps will be named by suburb and city or city and state.")]
        public int areaNameByStreetSuburb = BP.AreaNamingStreetSuburb.IntValue();

        [Range(0, 9999)] [Tooltip("Maps of an area (in Hectares) of more than this value will be named by city and state. " +
                                  "Smaller maps will be named by suburb and city or street and suburb.")]
        public int areaNameByCityState = BP.AreaNamingSuburbCity.IntValue();
        ...
        public void ResetToDefaults()
        {
            ...
            areaNameByStreetSuburb = BP.STD_AREA_NAMING_STREET_SUBURB;
            areaNameByCityState = BP.STD_AREA_NAMING_SUBURB_CITY;
            verbosity = 0;
        }
    }
    ...
    public class UBuildMap: MonoBehaviour
    {
        ...
        public void SetupBuildParameters()
        {
            ...
            if (FoundOsmFile(path) || LooksLikeLatLong(latLong))
            {
                ...
                BP.AreaNamingStreetSuburb.Set(parameters.areaNameByStreetSuburb);
                BP.AreaNamingSuburbCity.Set(parameters.areaNameByCityState);
                ...
                BF.Partitions.Set(options.roadNetworkPartitions);
                BF.NativeNames.Set(options.nativeNames);
